...
function QuickInvest({ s }: { s: ScalpSignal }) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="bottom" className="rounded-t-2xl pb-8 space-y-4 max-h-[90vh] overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="font-mono font-bold text-base" style={{ color: accent }}>
            {dir} {s.asset} — Demo Trade
          </SheetTitle>
        </SheetHeader>
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-mono text-muted-foreground">Balance: ${cash.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
        </div>
