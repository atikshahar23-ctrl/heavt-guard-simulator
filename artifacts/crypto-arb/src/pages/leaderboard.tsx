...
function Row({ entry, lang }: { entry: LeaderboardEntry; lang: import("@/lib/i18n").Lang }) {
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#9fb4c7]/55">
          {t("leaderboard.rank", lang)}{entry.rank}
        </div>
      </div>
      <div className="shrink-0 text-left">
        ...
        <div className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#9fb4c7]/45">
          {t("leaderboard.walletValue", lang)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div dir={lang === "he" ? "rtl" : "ltr"} className="relative min-h-dvh p-4 md:p-6 max-w-3xl mx-auto space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {entries.map((e) => (
            <Row key={e.rank} entry={e} lang={lang} />
          ))}
            ...
            <>
              <div className="flex items-center gap-2 px-1 pt-2">
                ...
                <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-[#9fb4c7]/45">
                  {t("leaderboard.yourRank", lang)}
                </span>
                <span className="h-px flex-1 bg-[#9fb4c7]/15" />
              </div>
              <Row entry={self} lang={lang} />
            </>
          )}
        </div>
      )}
