...
- **Single Render web service serves both API and frontend**: `render.yaml`'s `buildCommand` runs the root `pnpm run build`, which builds both `artifacts/api-server` (esbuild bundle) and `artifacts/crypto-arb` (Vite, output to `dist/public`). At runtime, `artifacts/api-server/src/app.ts` mounts `/api/*` first, then — if `artifacts/crypto-arb/dist/public` exists — serves it via `express.static` with an SPA fallback (`index.html`) for any other GET route, so client-side routing (wouter) works on full-page loads/refreshes. In local dev this directory doesn't exist (the frontend runs on its own Vite dev server), so the block is skipped entirely.
...
- **Risk Manager / auto-pause**: `evaluateRiskGuard` (in `autotrader-context.tsx`) pauses a bot on 3 consecutive losses, <25% win-rate after 5 trades, a daily loss cap, or a 25% drawdown. It runs for the 6 "extra" bots (Dip Buyer, Breakout Hunter, Blue-Chip DCA, Order Flow Bot, Range Bot, Technical Signals Bot) *and* for the Scalp Squad / Momentum bot via `autotrader-engine.tsx`'s own 30s risk-evaluation effect.
...
- Bot Command Center (`/bots`): paper-trading bots (incl. Dip Buyer, Breakout Hunter, Blue-Chip DCA, Order Flow Bot, Range Bot — a mean-reversion bot that trades against sharp short-term deviations from the rolling average price and banks the reversion — and Technical Signals Bot, a multi-indicator confluence bot that combines RSI(14) oversold/overbought extremes with a moving-average trend filter on large-cap stocks) with a master arm/disarm and an adaptive manager that nudges each bot's selectivity from its own rolling win-rate.
