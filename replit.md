...
- **Risk Manager / auto-pause**: `evaluateRiskGuard` (in `autotrader-context.tsx`) pauses a bot on 3 consecutive losses, <25% win-rate after 5 trades, a daily loss cap, or a 25% drawdown. It runs for the 5 "extra" bots (Dip Buyer, Breakout Hunter, Blue-Chip DCA, Order Flow Bot, Range Bot) *and* for the Scalp Squad / Momentum bot via `autotrader-engine.tsx`'s own 30s risk-evaluation effect.
...
- Bot Command Center (`/bots`): paper-trading bots (incl. Dip Buyer, Breakout Hunter, Blue-Chip DCA, Order Flow Bot, and Range Bot — a mean-reversion bot that trades against sharp short-term deviations from the rolling average price and banks the reversion) with a master arm/disarm and an adaptive manager that nudges each bot's selectivity from its own rolling win-rate.
